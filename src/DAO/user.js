...
export const isScreenIdUnique = async function (screenId, session) {
    const result = await User.findOne({ screenId }, {}, { session });
...
export const setTokenForAuth = async (email, userToken) => User.updateOne({ email }, { $set: { token: userToken } })

export const resetPass = async (email, salt, password) => User.updateOne({ email, snsType: 'normal' }, { salt, password })
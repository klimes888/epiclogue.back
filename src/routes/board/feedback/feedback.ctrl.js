    ...
    try {
      ...
      if (patch.ok === 1) {
        ...
        if (patch.n === 1 ) { 
          ...
          console.error(`[ERROR] 피드백 ${req.params.feedbackId} 가 존재하지않아 수정되지 않았습니다.`)
        ...
        console.error(`[ERROR] 피드백 ${req.params.feedbackId} 의 수정이 정상적으로 처리되지 않았습니다.`)
        return res.status(500).json({
          result: "error",
...
export const deleteFeedback = async (req, res, next) => {
  try {
    ...
    if (deletion.ok === 1) {
      ...
      console.error(`[ERROR] 피드백 ${req.params.feedbackId} 의 삭제가 정상적으로 처리되지 않았습니다.`)
      res.status(500).json({
        result: "error",
        message: "예기치 않은 문제가 발생했습니다.",
      });

 ...
 */
export const contentsWrapper = async (reqUserId, contentData, contentType, isForViewer) => new Promise(async (resolve, reject) => {
  if (!reqUserId) {
    resolve(contentData);
  }
  ...
  if (contentData) {
    ...
    if (contentType === 'Board') {
      ...
      if (isForViewer) {
        ...
        contentData.liked = !!likeIdSet.includes(contentData._id.toString());
        contentData.heartCount = await models.Like.countDocuments({
          targetInfo: contentData._id,
          targetType: 'Board',
        });
        contentData.bookmarked = !!bookmarkIdSet.includes(contentData._id.toString());
        ...
        const filteredFeedbacks = contentData.feedbacks.filter((each) => each.writer !== null);
        for (const eachFeedback of filteredFeedbacks) {
          eachFeedback.liked = !!likeIdSet.includes(eachFeedback._id.toString());
        ...
        const resultSet = [];
        // null check
        const filteredData = contentData.filter((each) => each.writer !== null);
      ...
      for (let userData of contentData) {
        ...
        if (reqUserId) {
          userData.isFollowing = userData._id === reqUserId
